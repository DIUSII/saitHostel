...
function link({
  ...
  bundle.traverseAssets(asset => {
    ...
    for (let dep of bundleGraph.getDependencies(asset)) {
      let resolved = bundleGraph.getDependencyResolution(dep); // If the dependency was deferred, the `...$import$..` identifier needs to be removed.
    ...
    ReferencedIdentifier(path) {
      ...
      if (imports.has(name)) {
        ...
        if (!imported) {
          ...
          node = replaceExportNode(asset, symbol, path); // If the export does not exist, replace with an empty object.
