...
var _default = {
  ...
  CallExpression: {
    ...
    exit(node, {
      ...
      let isRegisterServiceWorker = types.isStringLiteral(args[0]) && types.matchesPattern(callee, serviceWorkerPattern) && !(0, _utils2.hasBinding)(ancestors, 'navigator') && !isInFalsyBranch(ancestors);
      ...
      let isImportScripts = (asset.env.context === 'web-worker' || asset.env.context === 'service-worker') && callee.name === 'importScripts';
...
function isRequireAsync(ancestors, requireNode, ast) {
  ...
  if (functionParent && types.isCallExpression(functionParent) && types.isMemberExpression(functionParent.callee) && functionParent.callee.property.name === 'then' && isPromiseResolve(functionParent.callee.object)) {
    ...
    if (!types.isArrowFunctionExpression(parent) && !types.isReturnStatement(parent)) {
      ...
      functionParent.arguments[0] = types.isArrowFunctionExpression(fn) ? types.arrowFunctionExpression([], requireClone) : types.functionExpression(null, [], types.blockStatement([types.returnStatement(requireClone)])); // Add the original function as an additional promise chain
  ...
  if (functionParent && types.isCallExpression(parent) && types.isIdentifier(parent.callee) && types.isNewExpression(functionParent) && types.isIdentifier(functionParent.callee) && functionParent.callee.name === 'Promise' && types.isFunction(functionParent.arguments[0]) && types.isIdentifier(functionParent.arguments[0].params[0]) && parent.callee.name === functionParent.arguments[0].params[0].name) {
    return true;
  }
}

function isPromiseResolve(node) {
  return types.isCallExpression(node) && types.isMemberExpression(node.callee) && types.isIdentifier(node.callee.object) && node.callee.object.name === 'Promise' && node.callee.property.name === 'resolve';
}
...
function addURLDependency(asset, node, opts = {}) {
  let url = node.value;
  asset.addURLDependency(url, _objectSpread({
    loc: node.loc && (0, _utils.createDependencyLocation)(node.loc.start, node.value, 0, 1)
  }, opts));
